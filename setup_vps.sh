...
# 3. Créer .env backend (à remplir manuellement)
if [ ! -f ~/blind-guidance/backend/.env ]; then
    cat > ~/blind-guidance/backend/.env << 'EOF'
PORT=6111
GEMINI_API_KEY=your_gemini_api_key_here
EOF
    echo "⚠️  Edite ~/blind-guidance/backend/.env avec ta vraie GEMINI_API_KEY"
fi

# 4. Nginx
echo "🔧 Configuration Nginx..."
sudo cp /home/ubuntu/blind-guidance/nginx.conf /etc/nginx/sites-available/blind-guidance-6000
sudo ln -sf /etc/nginx/sites-available/blind-guidance-6000 /etc/nginx/sites-enabled/
sudo nginx -t && sudo systemctl reload nginx
echo "✅ Nginx configuré sur port 6000"
...
echo "✅ Service blind-guidance activé"

# 6. Install deps et démarrer
...
echo "════════════════════════════════════"
echo "✅ Setup terminé!"
echo "   Frontend: http://VPS_IP:6000"
echo "   Backend:  http://127.0.0.1:6111"
echo "════════════════════════════════════"
echo ""
echo "⚠️  N'oublie pas: nano ~/blind-guidance/backend/.env"
